        ...
        return (
            <div className={`flex items-center gap-4 py-2 ${className}`}>
                <div className="flex -space-x-2">
                    {[1, 2, 3, 4].map((i) => (
                        <div key={i} className="w-8 h-8 rounded-full border-2 border-[var(--bg-dark)] bg-[var(--card-bg)] flex items-center justify-center overflow-hidden">
                            <img src={`https://i.pravatar.cc/100?img=${i + 10}`} alt="User" className="w-full h-full object-cover opacity-80" />
                        </div>
                    ))}
                </div>
                <div className="flex flex-col">
                    <span className="text-sm font-black text-[var(--text-main)]">Trusted by 1,200+ Travelers</span>
                    <span className="text-[10px] text-[var(--text-muted)] font-bold uppercase tracking-widest">Global Community verified</span>
                </div>
            </div>
        );
    }
    ...
    if (type === 'verify') {
        return (
            <div className={`flex items-center gap-1.5 text-[var(--primary)] ${className}`}>
                ...
                <span className="text-[10px] font-black uppercase tracking-widest text-[var(--secondary)]">AI & Local Verified</span>
            </div>
        );
    }
